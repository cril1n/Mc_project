...
export default function OrderCheckOut({ route, navigation }) {
    ...
    const setAddressPost = async () => {
        try{
            setAddress(await ViewModel.getAddress(location.coords.latitude, location.coords.longitude));
        }catch(error){
            console.log(error);
        }
    ...
    return (
        ...
        <View style={styles.addressContainer}>
            ...
            <Text style={styles.addressText}>
                <Text style={styles.addressText2}>Region: </Text> {address.region + "\n" || " "} 
                <Text style={styles.addressText2}>City: </Text> {address.city + "\n" || " "} 
                <Text style={styles.addressText2}>Street: </Text> {address.street + "\n" || " "} 
                <Text style={styles.addressText2}>Street n°: </Text>{address.streetNumber || " "} 
            </Text> 
        </View>
        <View style={styles.content}>
            <Text style={styles.description}>
            Are you ready to confirm your order? Make sure your data
            are correct!
            </Text>
            <TouchableOpacity
